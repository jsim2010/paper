...
use mock::{ExplorerController, MockExplorer, MockUserInterface};
use pancurses::Input;
...
fn j_scrolls_down() {
    let mock_ui = MockUserInterface::new();
    let explorer_controller = Rc::new(RefCell::new(ExplorerController::default()));
    let file = File::new(
        Rc::new(MockExplorer::new(Rc::clone(&explorer_controller))),
        String::from("mock"),
    );
    explorer_controller
        .borrow_mut()

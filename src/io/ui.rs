...
pub(crate) use crossterm::{ErrorKind, event::{KeyCode as Key, KeyModifiers as Modifiers}};
...
pub(crate) trait Drain {
    ...
    fn iter(&self) -> SinkIter<'_, Self::Event, Self::Error>
    where Self: Sized,
    {
        SinkIter {
            sink: self,
...
impl TerminalDrain {
    pub(crate) fn new() -> Self {
        ...
        tx.send(get_terminal_size().into()).unwrap();
...
impl Drain for TerminalDrain {
    type Event = Input;
    type Error = RecvUserInputError;

    fn is_empty(&self) -> bool {
        let x = self.rx.is_empty() && match event::poll(INSTANT) {
    ...
    fn recv(&self) -> Result<Self::Event, Self::Error> {
        if self.rx.is_empty() {
            event::read().map(|event| event.into()).map_err(|e| e.into())
        } else {
...
#[derive(Debug, Error)]
pub enum RecvUserInputError {
    #[error("{0}")]
    Event(#[from] ErrorKind),
    #[error("{0}")]
